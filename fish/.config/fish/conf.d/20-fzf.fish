...
set -gx fzf_directory_opts --bind 'page-up:preview-up,page-down:preview-down,ctrl-d:preview-page-down,ctrl-u:preview-page-up,ctrl-/:change-preview-window(down|hidden|)'

# Detect clipboard command for history copy feature
...
set -gx fzf_history_opts --preview 'echo {}' --preview-window up:3:hidden:wrap --bind "ctrl-/:toggle-preview,ctrl-y:execute-silent(echo -n {2..} | $_clip_cmd)+abort" --color header:italic --header 'Press CTRL-Y to copy command into clipboard'
