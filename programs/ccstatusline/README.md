...
Claude Code status line, linked into `~` by home-manager from `programs/ccstatusline/` in the
checkout.
...
```
ccstatusline/
├── package.nix                      # The in-repo package: pinned tarball and hash
├── program.nix                      # Tier, install source, applied files
└── .config/ccstatusline/            # -> ~/.config/ccstatusline/
    ├── settings.json                # Widget layout, colors, global options -- live
    └── context-bar.sh               # Threshold-colored context-usage bar -- applied
```
...
2. **Point Claude Code at it.** Already set in `../claude/.config/claude/settings.json`:
   ...
   The bare name resolves through the Nix profile, which is on the `PATH` of anything the
   session starts.
