...
binaries. `mise-tasks/plugins` pins the bootstrapping copy of fisher to a release tag. To
bump a pin: pick the new ref (`gh api repos/Homebrew/install/commits/main --jq .sha`,
`gh release list -R jdx/mise`, `gh release list -R jorgebucaran/fisher`), download the file
at that ref, hash it with `sha256sum` (`shasum -a 256` on macOS), and change ref and hash
together — a stale hash fails loudly rather than running something else.
