...
# One activation in its three pieces, so `activate` and `setup` run the same code: whose home
# this is and what the activation will do to it (sms_activate_scan), the preview of that
# (sms_activate_show) and the activation itself (sms_activate_run). Sourced, never run.
#
...
# checked too: an activation run as another user in this home (sudo, say) builds cleanly
# for that user, and the generation already here is what says whose home it is.
...
# What activating $generation would do to this home, into the arrays the preview prints
...
sms_activate_scan() {
  ...
  # One extension for the whole activation, as home-manager takes one. `bak` unless a backup of
...
sms_activate_show() {
  sms_header "activate $configuration"
  sms_note "$generation"

  if [[ -n $current ]]; then
    sms_section packages "${#added[@]} to install, ${#removed[@]} to remove, against the current generation"
  else
    sms_section packages "${#added[@]} to install, no current generation: a first activation"
  fi
